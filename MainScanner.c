...
/* External objects */
extern uni_int line; /* Source code line numbers - defined in scanner.c */
 ...
 * -------------------------------------------------------------
 */
uni_null printScannerError(uni_string fmt, ...);
uni_null displayScanner(BufferPointer ptrBuffer);
uni_long getScannerFilesize(uni_string fname);
...
uni_int mainScanner(uni_int argc, uni_string* argv) {
	...
	sourceBuffer = readerCreate(READER_DEFAULT_SIZE, READER_DEFAULT_INCREMENT, MODE_ADDIT);
